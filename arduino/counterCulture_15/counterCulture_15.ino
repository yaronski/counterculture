...
void setup() {
  ...
  // A,  B,  C,  D,  E,  F,  G,  DP CHANGE [Display segment]
  // 7,  6,  4,  2,  1,  9, 10,  5         [Display pin]
  bool resistorsOnSegments = true;
  bool disableDecPoint = false;
  // 9V to the common pins (pin 8 & optionally pin 1) and ground to the segment pins
  ...
  pinMode(mainButton, INPUT_PULLUP);      // Main button with internal pull-up to move to the next sound file
...
void loop() {
  ...
  readSw = analogRead(rotaryInput) + 100;     // +100 to move away from the threshold of the adjacent language
